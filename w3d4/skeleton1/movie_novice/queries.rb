...
def pulp_fiction_actors
  ...
  # Actor.joins(:castings).joins(:movies).where('title = ?', 'Pulp Fiction').select("actors.*").distinct
...
def uma_movies
  ...
  Actor.joins(:movies).where('name = ?', 'Uma Thurman').order('yr ASC').select('movies.id, title, yr').distinct
end
